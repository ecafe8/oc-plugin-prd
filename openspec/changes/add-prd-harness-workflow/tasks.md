...
- [ ] 1.4 Add configuration loading and validation boundaries for workspace and user-level PRD harness settings
...
- [ ] 2.5 Define and validate the `.vibe/config.yaml` schema, including model roles, workflow defaults, and sync-related settings
- [ ] 2.6 Define change-request record schemas and deterministic file naming for `request.md`, `impact.md`, and `decision.md`
...
- [ ] 3.3 Implement feature-splitting workflow steps that scaffold `docs/features/feat-<slug>/` with the canonical document set and manifest metadata
- [ ] 3.4 Implement deterministic feature naming, collision handling, and `index.md` summary generation
...
- [ ] 4.5 Implement revision-loop behavior so rejected artifacts must re-enter review after edits before workflow advancement
...
- [ ] 6.5 Add workflow-level integration tests that drive a project from `project_discovery` through `implementation_in_progress` and through a `replan_required` recovery path
